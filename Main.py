...
for j in range(len(types)):#loop and check each column for non-numeric values
    name = types[j]
    if str(name) in ('object', 'str'): #finding column with object/string type
        le = LabelEncoder()
...
Y = dataset['labels'].values.ravel()#extract attack column as the target class value
Y = Y.astype('int')
...
@app.route('/')
def home():
    return '<h1> Cyber attack prediction </h1><a href="http://127.0.0.1:5000/index">index</a>'
